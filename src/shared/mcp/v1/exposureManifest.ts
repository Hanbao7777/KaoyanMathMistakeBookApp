...
export const mcpExternalExposureManifestVersion = 'mcp-external-exposure-v1@6' as const;
export const mcpExternalExposureManifest = Object.freeze({
  ...
  businessOperations: Object.freeze([
    ...
    'backups.delete',
    'database.restore',
    'database.replace_from_import',
    'database.clear_all',
    'imports.delete_batch',
    'data_root.migrate'
  ] as const)
});
...
export function assertMcpExternalExposureManifest(value: unknown): asserts value is typeof mcpExternalExposureManifest {
  ...
  if (!Object.isFrozen(manifest.businessOperations) || manifest.businessOperations.length !== 59 || new Set(manifest.businessOperations).size !== 59) throw new Error('MCP external exposure manifest is not exact');
}
