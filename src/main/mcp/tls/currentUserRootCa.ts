...
class PowerShellCurrentUserRootBackend implements CurrentUserRootBackend {
  async install(der: Uint8Array): Promise<void> {
    ...
    try { await execFileAsync('certutil.exe', ['-user', '-addstore', 'Root', file], { windowsHide: true, timeout: 15_000, maxBuffer: 32 * 1024 }); }
    finally { fs.rmSync(file, { force: true }); }
  }
  async remove(value: string): Promise<void> { await execFileAsync('certutil.exe', ['-user', '-delstore', 'Root', thumbprint(value)], { windowsHide: true, timeout: 15_000, maxBuffer: 32 * 1024 }).catch(() => undefined); }
