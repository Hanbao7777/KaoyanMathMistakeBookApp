...
class PowerShellCurrentUserRootBackend implements CurrentUserRootBackend {
  ...
  async remove(value: string): Promise<void> { await runCertutil(['-user', '-delstore', 'Root', thumbprint(value)]); }
...
export class CurrentUserRootCaLifecycle {
  ...
  async remove(thumbprintValue: string): Promise<void> {
    const value = thumbprint(thumbprintValue);
    const before = await this.backend.count(value);
    if (before === 0) return;
    if (before !== 1) throw new Error('CurrentUser Root CA removal is ambiguous');
    await this.backend.remove(value);
    if (await this.backend.count(value) !== 0) throw new Error('CurrentUser Root CA removal left a stale certificate');
  }
