...
const cookieName = 'kaoyan_oauth_continue';
...
function cookie(value: string): string { return `${cookieName}=${value}; Path=/oauth/authorize/; Secure; HttpOnly; SameSite=Strict`; }
function clearCookie(): string { return `${cookieName}=; Path=/oauth/authorize/; Secure; HttpOnly; SameSite=Strict; Max-Age=0`; }
function findCookie(header: string | undefined): string | undefined { return header?.split(';').map((part) => part.trim()).find((part) => part.startsWith(`${cookieName}=`))?.slice(cookieName.length + 1); }
...
export class LocalOAuthAuthorizationServer {
  ...
  async authorize(input: string | URLSearchParams | Record<string, unknown>): Promise<OAuthHttpResponse> {
    try {
      ...
      return { status: 202, headers: { ...noStore, 'content-type': 'text/html; charset=utf-8', 'set-cookie': cookie(capability) }, body: this.waitingPage(requestId) };
    } catch (error) {
  ...
  continue(requestId: string, capability: string | undefined): OAuthHttpResponse { const value = this.authorizeCapability(requestId, capability); if (!value) return { status: 404, headers: noStore }; this.expireOne(value); if (value.state === 'consumed') return { status: 410, headers: { ...noStore, 'set-cookie': clearCookie() } }; if (value.state !== 'approved' && value.state !== 'denied') return { status: 409, headers: noStore }; value.state = 'consumed'; const redirect = new URL(value.request.redirect_uri); if (value.code) redirect.searchParams.set('code', value.code); else redirect.searchParams.set('error', 'access_denied'); redirect.searchParams.set('state', value.request.state); return { status: 302, headers: { ...noStore, location: redirect.toString(), 'set-cookie': clearCookie() } }; }
...
export function oauthCapabilityFromCookie(header: string | undefined): string | undefined { return findCookie(header); }
