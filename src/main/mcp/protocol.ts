...
import { mcpV1Registry, mcpV1RegistryByName, createMcpCapabilitySummary } from './registry';
...
import { McpJobService } from './jobs';
...
export const mcpInitializeResult = Object.freeze({
  capabilities: Object.freeze({
    ...
    tasks: Object.freeze({ list: Object.freeze({}), cancel: Object.freeze({}), requests: Object.freeze({ tools: Object.freeze({ call: Object.freeze({}) }) }) })
  }),
  serverInfo: Object.freeze({ name: 'kaoyan-mcp', version: mcpServerVersion }),
  instructions: mcpServerInstructionsValue.text
});

export function createMcpInitializeResult(protocolVersion: string): Readonly<Record<string, unknown>> {
  if (protocolVersion === '2025-11-25') return mcpInitializeResult;
  return Object.freeze({ ...mcpInitializeResult, capabilities: Object.freeze({
    tools: Object.freeze({ listChanged: false }), resources: Object.freeze({ subscribe: false, listChanged: false }), prompts: Object.freeze({ listChanged: false })
  }) });
}
...
function toolDescriptor(toolName: unknown, principal: AgentPrincipal): McpRegistryDescriptor {
  ...
  if (!descriptor || descriptor.primitive !== 'tool' || !visibleToPrincipal(descriptor, principal.scopes)) {
...
function readResourceUri(uri: string): { readonly descriptor: McpRegistryDescriptor; readonly payload: JsonObject } {
  ...
  if (job) return { descriptor: mcpV1RegistryByName['jobs.view'], payload: { jobId: job[1] } };
...
async function callTool(options: McpProtocolOptions, jobs: McpJobService, request: JsonRpcRequest, principal: AgentPrincipal, tasksNegotiated: boolean): Promise<object> {
  try {
    ...
    if (requestParams.task !== undefined) {
      if (!tasksNegotiated || !requestParams.task || typeof requestParams.task !== 'object' || Array.isArray(requestParams.task) || Object.keys(requestParams.task).some((key) => key !== 'ttl')) throw new AgentError('VALIDATION_ERROR', { field: 'params.task' });
      return rpcResult(request.id, await jobs.createTask(descriptor, argumentsValue, (requestParams.task as Record<string, unknown>).ttl, principal));
    }
...
export function createMcpProtocolHandler(options: McpProtocolOptions): (input: {
  ...
  const jobs = new McpJobService({ gateway: options.gateway, randomUUID: options.randomUUID });
  return async ({ principal, tasksNegotiated = false, headers, request: rawRequest }) => {
    const request = rawRequest as unknown as JsonRpcRequest;
    try {
      if (request.method === 'tools/list') {
        const tools = mcpV1Registry.filter((descriptor) => descriptor.primitive === 'tool' && visibleToPrincipal(descriptor, principal.scopes)).map((descriptor) => ({
          name: descriptor.name, description: descriptor.description, inputSchema: toolInputSchema(descriptor),
          ...(tasksNegotiated && descriptor.exposure === 'business' && principal.scopes.includes('jobs.execute') ? { execution: { taskSupport: 'optional' } } : {})
        }));
        return { status: 200, body: rpcResult(request.id, { tools }) };
      }
      if (request.method === 'tools/call') return { status: 200, body: await callTool(options, jobs, request, principal, tasksNegotiated) };
      ...
      if (request.method.startsWith('tasks/')) {
        if (!tasksNegotiated) return { status: 200, body: rpcError(request.id, -32601, 'MCP Tasks were not negotiated') };
        const requestParams = params(request.params);
        if (request.method === 'tasks/get') return { status: 200, body: rpcResult(request.id, await jobs.get(name(requestParams.taskId, 'params.taskId'), principal)) };
        if (request.method === 'tasks/list') return { status: 200, body: rpcResult(request.id, await jobs.list(requestParams.cursor === undefined ? undefined : name(requestParams.cursor, 'params.cursor'), principal)) };
        if (request.method === 'tasks/cancel') return { status: 200, body: rpcResult(request.id, await jobs.cancel(name(requestParams.taskId, 'params.taskId'), principal)) };
        if (request.method === 'tasks/result') return { status: 200, body: rpcResult(request.id, await jobs.result(name(requestParams.taskId, 'params.taskId'), principal)) };
      }
