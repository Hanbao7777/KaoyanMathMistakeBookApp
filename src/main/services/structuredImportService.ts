...
import { createQuestion, linkQuestionKnowledgePoints } from './databaseService';
...
function toQuestionInput(row: StructuredImportRow): QuestionInput {
  return {
    title: row.title,
    content: row.content,
...
export async function confirmStructuredImport(sessionId: string): Promise<StructuredImportResult> {
  ...
  try {
    for (const row of session.preview.rows) {
      ...
      try {
        const saved = await createQuestion(toQuestionInput(row));
