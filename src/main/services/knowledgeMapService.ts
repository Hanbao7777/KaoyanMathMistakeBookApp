...
import { allSql, getDatabase, getQuestionsByIds, lastInsertId, oneSql, persistDatabase, runSql } from './databaseService';
...
function getDescendantNodeIds(database: Awaited<ReturnType<typeof getDatabase>>, nodeId: string) {
  const rows = allSql<Pick<KnowledgePoint, 'node_id' | 'parent_node_id'>>(database, 'SELECT node_id, parent_node_id FROM knowledge_points');
...
function upsertTextbook(database: Awaited<ReturnType<typeof getDatabase>>, textbookRaw: Record<string, unknown>, copied: { fileName: string; filePath: string }) {
  ...
  if (existing) {
    runSql(
      ...
      [asText(textbookRaw.subject), fileName, filePath || existing.file_path, asText(textbookRaw.note), now, existing.id]
    );
    return existing.id;
  }
  ...
  runSql(
    ...
    `INSERT INTO textbooks (title, subject, edition, file_name, file_path, note, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
    [title, asText(textbookRaw.subject), edition, fileName, filePath, asText(textbookRaw.note), now, now]
  );
  return lastInsertId(database);
}

function upsertKnowledgePoint(database: Awaited<ReturnType<typeof getDatabase>>, textbookId: number, raw: Record<string, unknown>) {
  const now = nowIso();
  ...
  if (existing) {
    runSql(
      ...
      `UPDATE knowledge_points SET
        textbook_id = ?, parent_node_id = ?, title = ?, category = ?, level = ?, sort_order = ?,
  ...
  runSql(
    ...
    `INSERT INTO knowledge_points (
      textbook_id, node_id, parent_node_id, title, category, level, sort_order,
      book_page, pdf_page, summary, core_formulas, common_question_types,
      common_error_reasons, tags, created_at, updated_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [textbookId, nodeId, ...params.slice(1), now]
  );
  ...
  try {
    ...
    try {
      ...
      for (const row of pointRows) {
        try {
          const status = upsertKnowledgePoint(database, textbookId, (row ?? {}) as Record<string, unknown>);
          if (status === 'imported') result.importedCount += 1;
...
export async function listKnowledgeTree(): Promise<KnowledgePointTreeNode[]> {
  const database = await getDatabase();
  const rows = allSql<KnowledgePoint>(database, 'SELECT * FROM knowledge_points ORDER BY level ASC, sort_order ASC, title ASC');
  const directCounts = buildQuestionCounts(database);
  const byId = new Map<string, KnowledgePointTreeNode>();

  for (const row of rows) {
    byId.set(row.node_id, { ...row, children: [], questionCount: directCounts.get(row.node_id) ?? 0 });
  }
...
export async function getKnowledgePointReviewStats(nodeId: string, includeChildren = true): Promise<KnowledgePointReviewStats | null> {
  const database = await getDatabase();
  const point = oneSql<KnowledgePoint>(database, 'SELECT * FROM knowledge_points WHERE node_id = ?', [nodeId]);
...
export async function listKnowledgeReviewStats(): Promise<KnowledgePointReviewStats[]> {
  const database = await getDatabase();
  const points = allSql<KnowledgePoint>(database, 'SELECT * FROM knowledge_points ORDER BY level ASC, sort_order ASC, title ASC');
...
export async function rematchKnowledgePoints(): Promise<KnowledgeRematchResult> {
  ...
  const points = allSql<KnowledgePoint>(database, 'SELECT * FROM knowledge_points ORDER BY level ASC, sort_order ASC');
