...
import type { DashboardData, KnowledgePointReviewStats, QuestionFilters } from '../../shared/types';
...
export function DashboardPage({
  ...
  useEffect(() => {
    Promise.all([window.api.dashboard(), window.api.listKnowledgeReviewStats()])
            ...
            {data.recent.map((question) => (
              <button type="button" key={question.id} onClick={() => onOpenQuestion(question.id)}>
                <div>
                  ...
                  <span>{question.category || '其他'} · {question.question_type || '其他'}</span>
                </div>
