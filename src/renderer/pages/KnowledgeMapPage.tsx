...
function filterKnowledgeTree(nodes: KnowledgePointTreeNode[], query: string, onlyWithQuestions: boolean): KnowledgePointTreeNode[] {
  const keyword = query.trim().toLowerCase();

  return nodes.flatMap((node) => {
    const children = filterKnowledgeTree(node.children, query, onlyWithQuestions);
    ...
    if (!matchesSearch || !matchesQuestions) return [];
          ...
          <div className="related-question-grid">
            {detail.relatedQuestions.map((question) => (
              <button className="related-question" type="button" key={question.id} onClick={() => onOpenQuestion(question.id)}>
                ...
                <span>{question.category} · {question.question_type}</span>
...
export function KnowledgeMapPage({ selectedNodeId, onOpenQuestion, onReviewKnowledgePoint }: KnowledgeMapPageProps) {
  ...
  const visibleTree = useMemo(() => filterKnowledgeTree(tree, search, onlyWithQuestions), [tree, search, onlyWithQuestions]);
  const totalNodeCount = useMemo(() => countNodes(tree), [tree]);
  const questionNodeCount = useMemo(() => countNodesWithQuestions(tree), [tree]);
  const statsByNode = useMemo(() => new Map(reviewStats.map((item) => [item.node_id, item])), [reviewStats]);
  const reviewSummary = useMemo(() => summarizeReviewStats(reviewStats), [reviewStats]);
