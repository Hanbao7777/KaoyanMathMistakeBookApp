import { ArrowLeft, CheckCircle2, Download, FileArchive, FileCheck2, LockKeyhole, RefreshCw, ShieldAlert, ShieldCheck, Trash2, UserRoundX } from 'lucide-react';
...
import { mcpClientScopeGroups } from '../../shared/mcp/v1/clientScopes';
import type { PairingProduct, PairingStatus } from '../../shared/mcp/v1/pairingContracts';
...
export function AgentControlCenterPage({ onBack }: { onBack: () => void }) {
  ...
  const [oauthConsent, setOauthConsent] = useState<Awaited<ReturnType<typeof window.api.agentControl.listOAuthConsent>>>([]);
  const [diagnosticPreview, setDiagnosticPreview] = useState<Awaited<ReturnType<typeof window.api.agentControl.previewDiagnostics>> | null>(null);
  ...
  async function act(key: string, action: () => Promise<unknown>, confirmation?: string) {
    ...
    try { const result = await action(); if (result !== false) toast('操作已记录到本地审计', 'success'); await load(); } catch (nextError) { setError(errorMessage(nextError)); } finally { setBusy(null); }
  }
  ...
  async function previewDiagnostics() {
    setBusy('diagnostic-preview'); setError('');
    try { setDiagnosticPreview(await window.api.agentControl.previewDiagnostics()); } catch (nextError) { setError(errorMessage(nextError)); } finally { setBusy(null); }
  }
  async function exportDiagnostics() {
    if (!await modal.confirm({ title: '导出本地诊断摘要', message: '导出仅包含版本、运行状态、审计计数和构建哈希，不包含错题内容、图片、数据库、凭据或绝对路径。', confirmLabel: '导出', danger: false })) return;
    setBusy('diagnostic-export'); setError('');
    try { const result = await window.api.agentControl.exportDiagnostics(); toast(`已导出 ${result.fileName}`, 'success'); setDiagnosticPreview(await window.api.agentControl.previewDiagnostics()); } catch (nextError) { setError(errorMessage(nextError)); } finally { setBusy(null); }
  }
  async function connect(product: PairingProduct) {
    const label = product === 'codex' ? 'Codex CLI' : 'Claude Code';
    if (!await modal.confirm({ title: `连接 ${label}`, message: `将安装已校验的 Kaoyan 启动器，并创建仅由启动器持有的 Windows CNG 密钥。App 只登记公钥。\n\n新客户端默认仅可读取；Scope 或信任升级需要在“已授权客户端”中另行保存。外部模型可能按未来授权范围接收学习数据。`, confirmLabel: '确认连接', danger: true })) return;
    setBusy(`connect-${product}`); setError('');
    try {
      const result = await window.api.agentControl.connectClient({ product, clientId: `${product}-${crypto.randomUUID().replace(/-/g, '').slice(0, 20)}`, requestedScopes: ['system.read'], trust: 'observer', disclosureAccepted: true, authorityConfirmed: false });
      setPairingStatus(result); toast(result.state === 'healthy' ? `${label} 已连接` : result.message, result.state === 'healthy' ? 'success' : 'error'); await load();
    } catch (nextError) { setError(errorMessage(nextError)); } finally { setBusy(null); }
  }
  async function connectionAction(product: PairingProduct, clientId: string, action: 'health' | 'repair' | 'rotate' | 'disconnect') {
    setBusy(`${action}-${clientId}`); setError('');
    try {
      const request = { product, clientId } as const;
      const result = action === 'health' ? await window.api.agentControl.getClientConnection(request)
        : action === 'repair' ? await window.api.agentControl.repairClientConnection(request)
          : action === 'rotate' ? await window.api.agentControl.rotateClientKey(request)
            : await window.api.agentControl.disconnectClientConnection(request);
      setPairingStatus(result); toast(result.message, result.state === 'healthy' || result.state === 'disconnected' ? 'success' : 'error'); await load();
    } catch (nextError) { setError(errorMessage(nextError)); } finally { setBusy(null); }
  }
  ...
  const directHttps = status?.directHttps;
  ...
  return <div className="page agent-control-page">
    ...
    <section className="agent-control-status">
      ...
      <label className="agent-toggle"><input aria-label="外部控制开关" type="checkbox" checked={Boolean(status?.settings.externalControlEnabled)} onChange={(event) => {
        const enabled = event.currentTarget.checked;
        void act('enabled', () => window.api.agentControl.setExternalControlEnabled(enabled), enabled ? '启用后，已授权外部客户端可按其 Scope 发起请求。' : '关闭将立即拒绝新的外部请求。');
      }} disabled={busy === 'enabled'} /><span /></label>
    </section>
    <section className="agent-control-section"><h2><LockKeyhole size={18} /> Direct HTTPS OAuth</h2>{directHttps ? <div className="agent-row compact"><div><strong>{directHttps.state === 'ready' ? '运行中' : directHttps.enabled ? '已配置但未运行' : '未启用'}</strong><small>{directHttps.authority} · Resource {directHttps.resource}</small><small>Issuer {directHttps.issuer} · App instance {directHttps.appInstanceId}</small>{directHttps.rootCaThumbprint ? <small>CurrentUser Root {directHttps.rootCaThumbprint}</small> : null}{directHttps.certificateThumbprint ? <small>Leaf certificate {directHttps.certificateThumbprint}</small> : null}{directHttps.reason ? <small>状态：{directHttps.reason}</small> : null}</div><span className={`agent-badge ${directHttps.state === 'ready' ? 'healthy' : ''}`}>{directHttps.state ?? (directHttps.enabled ? 'stopped' : 'disabled')}</span></div> : <div className="empty-state">Direct HTTPS OAuth 当前不可用。</div>}<div className="agent-actions">{!directHttps?.rootCaThumbprint ? <button className="primary-button" type="button" disabled={busy === 'https-trust'} onClick={() => void act('https-trust', async () => { const intent = await window.api.agentControl.prepareDirectHttpsTrust(); const confirmed = await modal.confirm({ title: '确认信任本地 HTTPS 根证书', message: `${intent.subject ?? 'Kaoyan Local HTTPS Root'}\n指纹：${intent.thumbprint}\n到期：${formatDate(intent.expiresAt)}\n\n仅安装到当前 Windows 用户的 Root 存储。`, confirmLabel: '安装并启用', danger: true }); await window.api.agentControl.confirmDirectHttpsTrust(intent.intentId, confirmed); return confirmed; })}>安装本地根证书</button> : <button className="secondary-button danger" type="button" disabled={busy === 'https-remove'} onClick={() => void act('https-remove', async () => { const intent = await window.api.agentControl.prepareDirectHttpsRemoval(); const confirmed = await modal.confirm({ title: '移除本地 HTTPS 根证书', message: `指纹：${intent.thumbprint}\n将停止 HTTPS OAuth 并撤销待授权。`, confirmLabel: '移除', danger: true }); await window.api.agentControl.confirmDirectHttpsRemoval(intent.intentId, confirmed); return confirmed; })}>移除本地根证书</button>}</div><p className="muted-text">固定 authority 只绑定 127.0.0.1；Root CA 仅在 CurrentUser 证书存储中按精确指纹管理，authority 变更会要求客户端重新注册。</p></section>
    <section className="agent-control-section"><h2><ShieldCheck size={18} /> OAuth 授权请求</h2>{oauthConsent.length ? oauthConsent.map((request) => <article className="agent-row compact" key={request.requestId}><div><strong>{request.clientDisplayName}</strong><small>{request.clientId} · {request.scopes.join(', ')}</small><small>{request.redirectDisplay} · {formatDate(request.createdAt)}</small></div><div className="agent-actions"><button className="primary-button compact-button" type="button" onClick={() => void act(`oauth-approve-${request.requestId}`, () => window.api.agentControl.decideOAuthConsent(request.requestId, 'approve'), '批准此客户端当前请求的 Scope。')}>批准</button><button className="secondary-button danger compact-button" type="button" onClick={() => void act(`oauth-deny-${request.requestId}`, () => window.api.agentControl.decideOAuthConsent(request.requestId, 'deny'), '拒绝此 OAuth 请求。')}>拒绝</button></div></article>) : <div className="empty-state">当前没有待处理的 OAuth 授权请求。</div>}</section>
    <section className="agent-control-section pairing-section"><h2><ShieldCheck size={18} />连接外部客户端</h2><p>选择后会先展示并记录数据披露与确认。启动器安装到用户本地稳定目录；私钥始终留在 Windows CNG，不会进入 App、IPC 或审计。</p><div className="agent-actions"><button className="primary-button" type="button" disabled={busy === 'connect-codex'} onClick={() => void connect('codex')}>连接 Codex CLI</button><button className="primary-button" type="button" disabled={busy === 'connect-claude_code'} onClick={() => void connect('claude_code')}>连接 Claude Code</button></div>{pairingStatus ? <div className={`pairing-result ${pairingStatus.state}`}><strong>{pairingStatus.product === 'codex' ? 'Codex CLI' : 'Claude Code'}：{pairingStatus.state}</strong><span>{pairingStatus.message}</span>{pairingStatus.launcherPath ? <code>{pairingStatus.launcherPath}</code> : null}{pairingStatus.manualConfiguration ? <code>{pairingStatus.manualConfiguration.executable} {pairingStatus.manualConfiguration.argv.map((argument) => JSON.stringify(argument)).join(' ')}</code> : null}</div> : null}</section>
    <section className="agent-control-section"><h2><ShieldCheck size={18} /> 已授权客户端</h2>{clients.length ? clients.map((client) => {
      ...
      const product = client.clientId.startsWith('codex-') ? 'codex' : client.clientId.startsWith('claude_code-') ? 'claude_code' : null;
      return <article className="agent-row agent-client-row" key={client.clientId}><div><strong>{client.displayName}</strong><small>{client.clientId} · 最近活动 {formatDate(client.lastActiveAt)}</small></div><label>信任<select value={draft.trust} onChange={(event) => { const trust = event.target.value; if (!isTrustProfile(trust)) return; setDrafts({ ...drafts, [client.clientId]: { ...draft, trust } }); }}>{trustProfiles.map((trust) => <option key={trust} value={trust}>{trustLabels[trust]}</option>)}</select></label><fieldset className="agent-scope-picker"><legend>Scope</legend>{mcpClientScopeGroups.map((group) => <section className="agent-scope-group" key={group.id}><strong>{group.label}</strong><small>{group.description}</small><div>{group.scopes.map((scope) => <label key={scope}><input type="checkbox" checked={draft.scopes.includes(scope)} onChange={(event) => setDrafts({ ...drafts, [client.clientId]: { ...draft, scopes: event.target.checked ? [...draft.scopes, scope] : draft.scopes.filter((value) => value !== scope) } })} />{scope}</label>)}</div></section>)}</fieldset><div className="agent-actions"><button className="secondary-button compact-button" type="button" disabled={busy === client.clientId} onClick={() => void act(client.clientId, () => window.api.agentControl.updateClientAccess(client.clientId, draft.scopes, draft.trust), `更新 ${client.displayName} 的 Scope 与信任级别。变更会立即终止其现有授权会话。`)}>保存访问</button>{product ? <><button className="secondary-button compact-button" type="button" onClick={() => void connectionAction(product, client.clientId, 'health')}>健康检查</button><button className="secondary-button compact-button" type="button" onClick={() => void connectionAction(product, client.clientId, 'repair')}>修复</button><button className="secondary-button compact-button" type="button" onClick={() => void connectionAction(product, client.clientId, 'rotate')}>轮换密钥</button><button className="secondary-button danger compact-button" type="button" onClick={() => void connectionAction(product, client.clientId, 'disconnect')}>断开</button></> : null}<button className="icon-button danger" title="撤销客户端" type="button" disabled={busy === `revoke-${client.clientId}`} onClick={() => void act(`revoke-${client.clientId}`, () => window.api.agentControl.revokeClient(client.clientId), `撤销 ${client.displayName} 后，该客户端的新请求会立即失效。`)}><UserRoundX size={16} /></button></div></article>;
    ...
    <section className="agent-control-section"><h2><FileArchive size={18} /> 本地诊断</h2><div className="agent-actions"><button className="secondary-button" type="button" disabled={busy !== null} onClick={() => void previewDiagnostics()}><FileArchive size={16} />预览</button><button className="primary-button" type="button" disabled={busy !== null} onClick={() => void exportDiagnostics()}><Download size={16} />导出 ZIP</button></div>{diagnosticPreview ? <div className="agent-audit-list">{diagnosticPreview.files.map((file) => <div key={file.name}><strong>{file.name}</strong><span>{file.bytes} bytes · {file.sha256.slice(0, 16)}...</span></div>)}<div><strong>合计</strong><span>{diagnosticPreview.totalBytes} bytes · {formatDate(diagnosticPreview.generatedAt)}</span></div></div> : <div className="empty-state">尚未生成诊断预览。</div>}</section>
