...
export function AgentControlCenterPage({ onBack }: { onBack: () => void }) {
  ...
  const directHttps = (status as (AgentControlStatus & { readonly directHttps?: { readonly port: number; readonly authority: string; readonly resource: string; readonly issuer: string; readonly appInstanceId: string; readonly enabled: boolean; readonly certificateThumbprint?: string; readonly rootCaThumbprint?: string } | null }))?.directHttps;
  ...
  return <div className="page agent-control-page">
    ...
    <section className="agent-control-section"><h2><LockKeyhole size={18} /> Direct HTTPS OAuth</h2>{directHttps ? <div className="agent-row compact"><div><strong>{directHttps.enabled ? '已配置' : '未启用'}</strong><small>{directHttps.authority} · Resource {directHttps.resource}</small><small>Issuer {directHttps.issuer} · App instance {directHttps.appInstanceId}</small>{directHttps.rootCaThumbprint ? <small>CurrentUser Root {directHttps.rootCaThumbprint}</small> : null}{directHttps.certificateThumbprint ? <small>Leaf certificate {directHttps.certificateThumbprint}</small> : null}</div><span className={`agent-badge ${directHttps.enabled ? 'healthy' : ''}`}>{directHttps.enabled ? 'ready' : 'disabled'}</span></div> : <div className="empty-state">Direct HTTPS OAuth 当前不可用。</div>}<p className="muted-text">固定 authority 只绑定 127.0.0.1；Root CA 仅在 CurrentUser 证书存储中按精确指纹管理，authority 变更会要求客户端重新注册。</p></section>
